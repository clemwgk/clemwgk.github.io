...
The good news is that today's LLMs (or maybe more accurately the harnesses and tools) also enable better agentic QA. The best QA will always be the human, but having an agent spin up subagents to QA its own work is a reasonable compromise, and I imagine that's the way forward for low-stakes tasks, reserving human QA for the highest stakes. I can see myself doing exactly that: if I spend my own time QA-ing the workflow on every run, did the automation really save me any bandwidth? The point is to tier the QA and check the output _selectively_.

For a routinely scheduled automation, monitoring becomes equally important. How do we even know the process ran at all? The process needs to be suitably noisy, so that process failures are noisy too, rather than silent and undetected. I recently started trialing a script to auto-archive and delete emaisl in my Gmail inbox, and I have it email me after each run as a kind of "proof of work" (an artifact!). Failures in the process would be "noisy" (visible) - the moment that email says nothing happened, or the email doesn't arrive on schedule at all.
